"""Tests for ``anna_web.routes.schedule_routes`` (subtask 9).

Eight cases per the buildout plan:

1. ``GET /schedules`` renders a list with every existing schedule.
...
# ---------------------------------------------------------------------------
# 1. GET /schedules renders the list.
# ---------------------------------------------------------------------------
...
def test_post_creates_schedule(anna_home: Path) -> None:
    ...
    # The row partial is returned; the id should appear.
